...
pub(crate) const DOCUMENTABLE_ANYWHERE: &[&str] = &[
    ...
    //
    // `type_elem` is scoped by [`is_documentable`] rather than listed unconditionally: the grammar
    // also puts one under `type_arguments`, so unscoped it makes every generic type argument a
    // documentable subject in its own right.
    "field_declaration",
    "method_elem",
...
impl Pack for GoPack {
    ...
    fn visibility(&self, subject: Node, src: &str) -> Visibility {
        ...
        // An embedded interface element names no `name` field at all, so it would otherwise fall to
        // the `None` arm and read as package-restricted whatever it embeds — silencing
        // `implInInterface` on every embedded exported interface. Its visibility is its type's.
        if subject.kind() == "type_elem" {
            let mut cursor = subject.walk();
            let embedded = subject.named_children(&mut cursor).next();
            return match embedded.map(|n| src[n.byte_range()].trim_start_matches('*')) {
                Some(text) => match text.rsplit('.').next().filter(|s| !s.is_empty()) {
...
/// The specs a `const` or `var` block declares.
///
/// The nesting branch is `var`'s alone: `var_declaration` holds either a `var_spec` or a
/// `var_spec_list`, while `const_declaration` lists `const_spec` children directly in both its
/// single and its grouped form. Written for both kinds because the asymmetry is the grammar's and
/// not worth a second function, but a test that exercises only `const` leaves the branch untouched.
